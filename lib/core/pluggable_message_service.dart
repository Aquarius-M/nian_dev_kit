...
class PluggableMessageService {
  static final PluggableMessageService _instance = PluggableMessageService._internal();
  factory PluggableMessageService() {
    return _instance;
  }

  // ignore: close_sinks
  StreamController<PluggableMessage> messageStreamController = StreamController.broadcast();

  Map<String, PluggableMessageInfo> get pluggableMessageData => _pluggableMessageData;
  ...
  void resetListener() {
    ...
    PluginManager.instance.pluginsMap.values.whereType<PluggableWithStream>().forEach((element) {
  ...
  int count(Pluggable pluggable) => _pluggableMessageData[pluggable.name]?.count ?? 0;
  ...
  void _sendSink(Pluggable pluggable) {
    messageStreamController.sink.add(PluggableMessage.create(pluggable.name, PluggableMessageService().count(pluggable)));
  }
}
