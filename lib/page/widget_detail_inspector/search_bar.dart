...
class _SearchInputState extends State<SearchBar> {
  ...
  Widget _buildTextField() {
    ...
    if (widget.inputCharactersLength > 0) {
      formatters = [LengthLimitingTextInputFormatter(widget.inputCharactersLength)];
    }
  ...
  Widget _buildInput() {
    return Container(
      decoration: widget.style == BarStyle.border ? BoxDecoration(color: Colors.white, borderRadius: const BorderRadius.all(Radius.circular(6.0)), border: Border.all(color: Colors.black12, style: BorderStyle.solid, width: 1)) : BoxDecoration(color: Colors.black.withOpacity(0.04), borderRadius: const BorderRadius.all(Radius.circular(6.0))),
  ...
  Widget _buildClickButton() {
    ...
    if (widget.right != null) {
      right = Container(constraints: const BoxConstraints(maxHeight: 38), padding: const EdgeInsets.only(left: 20.0, right: 20), child: widget.right);
    } else {
